...
server.listen(config.port, () => {
  ...
  console.log(
    `  models: simulation=${config.models.simulation} extraction=${config.models.extraction} debrief=${config.models.debrief} eqPrep=${config.models.eqPrep}`,
  );
  console.log(
    `  voice: ${config.mockAi ? "mock relay" : config.voice.openaiApiKey ? `${config.voice.model} / ${config.voice.voice}` : "not configured (OPENAI_API_KEY missing)"}`,
  );
  if (config.mockAi) {
