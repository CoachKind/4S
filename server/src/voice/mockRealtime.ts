 ...
 * using the same event names the real service sends. This lets the browser
 * flow, the relay, and the transcript accumulation run without a key.
 */
...
export class MockRealtimeUpstream extends EventEmitter implements RealtimeUpstream {
  ...
  send(event: RealtimeEvent): void {
    ...
    switch (event.type) {
      ...
      case "conversation.item.create": {
        ...
        this.emit("message", { type: "conversation.item.created", item: { id: item.id ?? newId(), type: "message", role: item.role ?? "user" } });
        return;
      }
      default:
        return;
  ...
  private runTurn() {
    ...
    const steps: Array<[number, RealtimeEvent]> = [
      ...
      [440, { type: "conversation.item.created", item: { id: userItem, type: "message", role: "user" } }],
      ...
      [1000, { type: "conversation.item.created", item: { id: assistantItem, type: "message", role: "assistant" } }],
      [1100, { type: "response.audio.delta", response_id: responseId, item_id: assistantItem, delta: silence(0.4) }],
      [1300, { type: "response.audio.delta", response_id: responseId, item_id: assistantItem, delta: silence(0.4) }],
      [1500, { type: "response.audio_transcript.done", response_id: responseId, item_id: assistantItem, transcript: REPLIES[i] }],
      [1520, { type: "response.audio.done", response_id: responseId, item_id: assistantItem }],
