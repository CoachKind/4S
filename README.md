...
**Relay.** The browser opens a WebSocket to `/voice/:sessionId/connect`; the server opens one to the OpenAI Realtime API and relays between them. The OpenAI key never reaches the browser. On connect the server sends a GA `session.update` (`session.type: "realtime"`, `output_modalities: ["audio"]`, and an `audio` object with 24kHz `audio/pcm` in and out, `gpt-4o-mini-transcribe` input transcription, server-side voice activity detection, and the configured voice) carrying the persona prompt plus a spoken-conversation addition as `instructions`. Any prior transcript is seeded into the model's context as GA conversation items (`input_text` for the user, `output_text` for the simulated person) so a switch or reconnect keeps continuity. The browser may only send audio buffer events, cancel, and truncate; it can never change the session configuration. The beta interface and the `gpt-4o-realtime-preview` models were retired in May 2026 and are not supported.

**Errors.** Upstream `error` events are forwarded to the browser as `relay.error` with the service's message. Before the relay reports ready they are marked fatal ("Voice mode couldn't start…") and the client tears down cleanly; afterwards they read "[Name] couldn't respond…". The server logs only the error's type, code, param, and message, never the whole event, and nothing from the Emotional Check-In is ever part of a voice session.

**Transcript.** Turns are placed when the service creates conversation items (`conversation.item.added`, finalized by `conversation.item.done`) and filled in when `conversation.item.input_audio_transcription.completed` (user) and `response.output_audio_transcript.done` (simulated) arrive, so order follows speech even when the user's transcription lands after the reply starts. Each completed turn is pushed to the browser as `relay.transcript` and saved to the session; the transcript is saved again when the connection closes. The debrief reads it exactly as it reads a text transcript, and adds one line to its system prompt asking the coach to consider delivery.
