      ...
      <div className="space-y-2">
        ...
        {value.map((f, i) => (
          <div key={i} className="grid grid-cols-[1fr_72px_auto] gap-2">
            ...
            />
            <Button variant="ghost" className="px-2 py-1 text-xs" onClick={() => onChange(value.filter((_, idx) => idx !== i))} aria-label="Remove">
              ✕
            </Button>
            <input
      ...
      <main className="mx-auto max-w-4xl px-5 py-8">
        <div className="mb-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
            This is the data extracted from the PDF. Check it against the report and correct anything that looks wrong.
            The simulation and debrief will use exactly what you confirm here.
          </p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <Card className="p-5">
            ...
            <div className="grid gap-6 lg:grid-cols-3">
              <ForceEditor label="Primary" value={draft.driving_forces.primary} onChange={(v) => setDraft({ ...draft, driving_forces: { ...draft.driving_forces, primary: v } })} />
              <ForceEditor label="Situational" value={draft.driving_forces.situational} onChange={(v) => setDraft({ ...draft, driving_forces: { ...draft.driving_forces, situational: v } })} />
              <ForceEditor label="Indifferent" value={draft.driving_forces.indifferent} onChange={(v) => setDraft({ ...draft, driving_forces: { ...draft.driving_forces, indifferent: v } })} />
            </div>
          </Card>
          ...
          <Card className="p-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <ListEditor label="Top 5" value={draft.competencies.top_5} onChange={(v) => setDraft({ ...draft, competencies: { ...draft.competencies, top_5: v } })} />
              <ListEditor label="Bottom 5" value={draft.competencies.bottom_5} onChange={(v) => setDraft({ ...draft, competencies: { ...draft.competencies, bottom_5: v } })} />
            </div>
          </Card>
          ...
          <Card className="p-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <ListEditor label="Under pressure" value={draft.behavioral_flags.under_pressure} onChange={(v) => setDraft({ ...draft, behavioral_flags: { ...draft.behavioral_flags, under_pressure: v } })} />
              <ListEditor label="Areas for improvement" value={draft.behavioral_flags.areas_for_improvement} onChange={(v) => setDraft({ ...draft, behavioral_flags: { ...draft.behavioral_flags, areas_for_improvement: v } })} />
              <ListEditor label="Ways to communicate" value={draft.behavioral_flags.communication_do} onChange={(v) => setDraft({ ...draft, behavioral_flags: { ...draft.behavioral_flags, communication_do: v } })} />
              <ListEditor label="Ways NOT to communicate" value={draft.behavioral_flags.communication_dont} onChange={(v) => setDraft({ ...draft, behavioral_flags: { ...draft.behavioral_flags, communication_dont: v } })} />
            </div>
          </Card>
        </div>
